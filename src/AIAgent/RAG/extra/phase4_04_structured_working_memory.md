...
这一层仍然是线程级工作记忆；不指定 `--structured-state-file` 时只存在于进程内，指定后可作为线程 checkpoint 跨进程恢复。它不等于按相关性检索的长期语义库；后续 [Phase 4.6](phase4_06_semantic_memory.md) 会把 `fact` 路由到独立存储，并在需要时召回。
...
* **语义记忆 (Semantic Memory) —— 负责“持久化并按需激活的稳定事实”**
  * `SemanticMemory` 将 `fact` 保存到版本化 JSON 和向量索引中；自动召回或 `search_semantic_memory` 工具只把本轮相关事实追加到 Prompt 尾部。
