...
Phase 2 🚧 进阶 RAG
   ...
   ├─ 第二块 ⬜ Reranking（Cross-encoder 二次精排）  ← 下一步
...
| `hybrid_retriever.py` | RRF 纯函数 + 编排类 | `reciprocal_rank_fusion(lists, k, top_k)` / `HybridRetriever(embedder, dense_store)` |

### 接口对齐（刻意和 Phase 1 一致，便于融合）
- 两种检索器的 `search` 都返回 `[{"chunk": Chunk, "score": float}, ...]`
