...
### 决策3：Multi-Query 的 RRF 合并复用了 hybrid_retriever 里的 `reciprocal_rank_fusion`
...
Hybrid 召回（Dense + BM25 + RRF）
    ...
    │
    ▼
Cross-encoder Rerank 精排       [reranker.py]
    │  始终用原始问题打分
    │
    ▼
...
| 文件 | 内容 | 关键接口 |
|:---|:---|:---|
| `query_rewriter.py` | 三种策略实现 | `QueryRewriter(llm_client, model).rewrite(q)` / `.hyde(q)` / `.multi_query(q, n)` |

### 修改文件

| 文件 | 改动 |
|:---|:---|
| `rag_chain.py` | `import reciprocal_rank_fusion` 从 hybrid_retriever；`__init__` 新增 `query_rewrite` 参数；`query()` Step 0 插入改写；`_build_response()` 抽出为独立方法（供 multi_query 路径复用） |

### 跑

```bash
uv run python query_rewriter.py              # 单测三种策略（纯 LLM，不需要知识库）
...
> 新对话开场建议说：「我在学 Phase 2，前三块 Hybrid / Reranking / Query Rewriting 已落地（见 phase2_hybrid_search.md / phase2_reranking.md / phase2_query_rewriting.md），现在要做第四块评估（RAGAS）」。
