...
把两条腿合起来：
  ...
  - Sparse（BM25，sparse_retriever.py）：精确命中关键词、专名、编号
...
def _chunk_key(chunk: Chunk) -> str:
    ...
    优先用 (source, chunk_index)（来自分块时写入的元数据），
    没有就退化到用内容本身。
    """
...
def reciprocal_rank_fusion(
    ...
    Args:
        result_lists: 多路结果，每路是 [{"chunk": Chunk, "score": float}, ...]
                      （已按各自的相关度降序排好）
        k: RRF 常数，越大则越淡化名次差距
        top_k: 融合后返回多少条

    Returns:
        融合后的结果 [{"chunk": Chunk, "score": rrf_score, "ranks": {...}}, ...]
...
class HybridRetriever:
    ...
    用法（注入你 Phase 1 的 embedder 和向量库）：
        hybrid = HybridRetriever(embedder, dense_store)
        hybrid.add(chunks, vectors)          # 同时灌进向量库和 BM25
        results = hybrid.search("问题", top_k=3)

    设计要点：
      - dense_store 复用你已有的 SimpleVectorStore / ChromaVectorStore，
        接口都是 .add(chunks, vectors) / .search(vector, top_k)
      - BM25 由本类内部持有，不需要外部关心
    """

    def __init__(self, embedder, dense_store, rrf_k: int = 60):
    ...
    def add(self, chunks: list[Chunk], vectors: list[list[float]]):
        """同一批 chunk 同时灌入两个索引（向量库要 vectors，BM25 不要）"""
        self.dense_store.add(chunks, vectors)
    ...
    def search(
        ...
        Returns:
            RRF 融合后的 [{"chunk", "score", "ranks"}, ...]
        """
        candidate_k = candidate_k or top_k * 3

        # ---- 第 1 路：Dense（向量检索）----
        query_vector = self.embedder.embed_query(query)
        dense_results = self.dense_store.search(query_vector, top_k=candidate_k)

        # ---- 第 2 路：Sparse（BM25）----
        sparse_results = self.sparse_store.search(query, top_k=candidate_k)

        if verbose:
            print("  [Dense 召回]")
            for i, r in enumerate(dense_results, 1):
                print(f"    {i}. ({r['score']:.4f}) {r['chunk'].content[:50]}...")
            print("  [Sparse 召回]")
...
if __name__ == "__main__":
    ...
    hybrid = HybridRetriever(embedder, SimpleVectorStore())
    hybrid.add(chunks, vectors)
